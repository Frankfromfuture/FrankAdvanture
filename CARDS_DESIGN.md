...
> **总规模：22 张**（起始手牌 7 + 起始牌堆 15）
> **每月抽牌**：3 张（`GAME_CONFIG.drawPerMonth`）
> **手牌上限**：10 张
> **理论数学**：6 局轮转 + 招聘市场补充 + 冷却回流 ≈ 总曝光 35+ 张
...
|---|---|---|
| 1-3 | 3 张 | 当前实装 |
| 4-7 | 3 张 | 当前实装；后续可随难度扩展 |
| 8-10 | 3 张 | 当前实装；后续可随难度扩展 |

刷新规则：
- 从当前关卡可入池的卡（`unlock_level ≤ 当前关卡`）随机抽
- 按稀有度概率抽取（见 §12.2）
- 每张刷出时 **roll 数值**（如稀有度允许，roll 词条）
- **明牌三选一**：3 张完整展示，玩家每月最多签 1 张，签约后本月市场关闭
- 卡面显示本次 roll 偏移（如 `+18%` / `-12%`），用于快速比较装备感
